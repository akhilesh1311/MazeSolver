...
class Astar(object):
  ...
  def __init__(self, maze, heuristic_type="E"):
...
#            print(self.heuristic[(i,j)],' ',i,' ',j)
  ...
  def traverse(self):
    ...
    while curr_i != (self.dim-1) or curr_j != (self.dim-1):
      ...
      curr_i = curr_node.i
      curr_j = curr_node.j
#      self.travelled_path.append((curr_i, curr_j))
#      print(curr_i, ' ', curr_j, ' ', curr_node.total_cost)
...
class DFSSolver(Maze):
    ...
    def find_path(self, cur_i, cur_j):
        ...
        while self.dfs_stack:
            ...
            if len(self.dfs_stack) > self.max_fringe_size:
                self.max_fringe_size = len(self.dfs_stack)
            self.total_fringe_size = self.total_fringe_size + len(self.dfs_stack)
                ...
                for j in range(cur_j - 1, cur_j + 2):
                    if 0 <= i < self.dim and 0 <= j < self.dim and abs(
                        ...
                        if self.arr[i][j] == 'G':
                            ...
                            self.avg_fringe_size = self.total_fringe_size/self.iterations
                            return
        self.avg_fringe_size = self.total_fringe_size/self.iterations
